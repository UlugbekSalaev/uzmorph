...
class UzMorph:
    ...
    def __init__(self, data_path=None):
        if data_path is None:
            # Get the directory where this script is located, then look in data/
    ...
    def analyze(self, word, pos_filter=None):
        ...
        if word in self.root_lexicon:
            for word_pos in self.root_lexicon[word]:
        ...
        if word in self.non_affixed:
            for word_pos in self.non_affixed[word]:
                if not pos_filter or word_pos == pos_filter:
                    res = {
                        "word": word, "stem": word, "lemma": word,
                        "cse": None, "cse_formula": None
    ...
    def _lemma_map_check(self, soz, pos_filter=None):
        ...
        for (word, lemma, affix) in self.lemma_map:
            ...
            if not toliq_cse:
                lemma_pos_set = self.root_lexicon.get(lemma, {"NOUN"})
                for lemma_pos in lemma_pos_set:
    ...
    def _cse_stripping(self, soz, pos_filter=None):
        ...
        for allomorf in self.tartiblangan_allomorflar:
            ...
            istisno_pos_set = self.exception_stems.get(stem, set())
            lugat_pos_set = self.root_lexicon.get(stem, set())
            
            lugatda = bool(lugat_pos_set)
            istisno = bool(istisno_pos_set)
            ...
            for qoida_idx in self.allomorf_xarita[allomorf]:
                ...
                # POS VALIDATION based on lexicon
                # If the stem is in the lexicon, verify if the rule POS matches one of the lexicon POS
                if lugatda and qoida_pos not in lugat_pos_set:
                    # Exception: Some suffixes might change POS (derivational), 
                    # but current cse.csv mostly has inflectional flags.
                    continue
